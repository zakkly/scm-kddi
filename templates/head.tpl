...
<link href="{_BASE_URL_}/css/{$v}.css?{time()}" rel="stylesheet">
	{/foreach}
{/if}

{if $smarty.session.role != "admin"}
<link href="{_BASE_URL_}/css/User.css?{time()}" rel="stylesheet">
{/if}
...
{/if}
{if $smarty.session.role != "admin"}
<script src="{_BASE_URL_}/js/User/user.js?{time()}" type="text/javascript"></script>
{/if}
